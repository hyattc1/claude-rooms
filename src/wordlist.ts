// EFF Short Wordlist 1, filtered to 3-5 char lowercase ASCII entries.
// Source: https://www.eff.org/files/2016/09/08/eff_short_wordlist_1.txt
// EFF maintains this list under CC-BY 3.0 for passphrase use; it is curated
// for pronounceability, short length, and absence of homophones, slurs,
// and near-duplicates.
//
// 1295 entries here. With the default 4-word room code, the keyspace is
// 1295^4 ~= 2.8 trillion combinations. Casual brute force becomes
// infeasible at that scale.
//
// Bundled into the build at compile time; no runtime fetch.
export const WORDLIST: readonly string[] = Object.freeze([
  ...
  "yard", "year", "yeast", "yelp", "yield", "yodel", "yoga", "yoyo",
  "yummy", "zebra", "zero", "zesty", "zippy", "zone", "zoom", 
]) as readonly string[];

// Validate at import time. If anyone edits the list and accidentally
// duplicates an entry or breaks the length contract, we want to know
// loudly during boot, not silently at room-code-generation time.
{
  ...
  for (const w of WORDLIST) {
    ...
    if (!/^[a-z]+$/.test(w)) {
      throw new Error(`wordlist entry not lowercase ASCII: ${w}`);
    }
    if (w.length < 3 || w.length > 5) {
      throw new Error(`wordlist entry not 3-5 chars: ${w}`);
    }
    seen.add(w);
  }
  if (WORDLIST.length < 512) {
    throw new Error(`wordlist below safety floor (${WORDLIST.length} < 512)`);
  }
}
